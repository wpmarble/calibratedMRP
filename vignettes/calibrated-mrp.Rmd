...
Formally, we observe a 3-dimensional array $\tilde{\Theta}$ with entries $\tilde{\theta}_{(m, c, j)}$ representing a draw $m$ from the posterior distribution of the share of respondents in poststratification cell $c$ who answered "yes" to question $j$.
...
Let's start by visualizing the modeled relationship between the outcome and the demographic predictors. We'll start by plotting all poststratifcation cells in Pennsylvania for the Biden legitimacy outcome. Each cell represents a combination of county, age group, race, gender, and education level. 
