...
```
# Generate MRP Estimates

Now that we have a model relating attitudes to demographic and geographic features, the next step is to generate estimates for every cell in the poststratification table. Since we are operating in a Bayesian setting, we will generate estimates for each posterior draw. The `posterior_epred()` function from `brms` allows us to generate predictions for new data, which we will use to generate estimates for each poststratification cell in `ps_cty`. This function returns an array that has dimensions $\text{(N draws)} \times \text{(N poststratification cells)} \times \text{(N outcomes)}$. This step is computationally intensive.^[3]

^[3]: For computational efficiency, we will only use a subset of posterior draws to generate predictions. In practice, you would want to use all posterior draws, but this is less memory-intensive and faster for demonstration purposes.

Formally, we observe a 3-dimensional array $\tilde{\mathcal{Y}}$ with entries $\tilde{\theta}_{(m, c, j)}$ representing a draw $m$ from the posterior distribution of the share of respondents in poststratification cell $c$ who answered "yes" to question $j$.


```{r generate-predictions, cache=TRUE}
draw_ids <- sample(1:ndraws(mod), 100)  # subset of posterior draws to use

# Generate predictions for poststratification cells. 
# Larger poststratification tables require more compute time. 
...
```{r plug-in-uncalibrated}
# get posterior mean for each cell
pred_plugin <- apply(pred_draws, c(2, 3), mean)
dimnames(pred_plugin)[[2]] <- dimnames(pred_draws)[[3]]  # use outcome names from model

# convert to tibble
pred_plugin <- as_tibble(pred_plugin) %>% 
  rename_with(~ paste0(.x, "_uncalib")) %>% 
  mutate(cell = row_number()) 

# get posterior SD 
pred_se <- apply(pred_draws, c(2, 3), sd)
dimnames(pred_se)[[2]] <- dimnames(pred_draws)[[3]]  # use outcome names from model
pred_se <- as_tibble(pred_se) %>% 
  rename_with(~ paste0(.x, "_uncalib_se")) %>% 
  mutate(cell = row_number()) 

# combine predictions and SEs
pred_plugin <- left_join(pred_plugin, pred_se, by = "cell")
celln <- which(names(pred_plugin) == "cell")
order <- c("cell", sort(names(pred_plugin)[-celln]))
pred_plugin <- pred_plugin[, order] 

# combine with original poststratification table
ps_cty <- bind_cols(ps_cty, pred_plugin %>% select(-cell))
```

Let's begin visualizing the relationship between the outcome and the demographic predictors. We'll start by plotting all poststratifcation cells in Pennsylvania for the Biden legitimacy outcome. Each cell represents a combination of county, age group, race, gender, and education level. 

```{r, fig.height=10, fig.width=9}
ggplot(ps_cty %>% filter(state == "PA")) + 
  aes(y = bidenlegitimate_uncalib, x = as.integer(agegrp), 
...
The `poststratify()` function generates state-level estimates. 

```{r uncalib-state, fig.height=9, fig.width=6}
stop("need to check the se functionality here")
